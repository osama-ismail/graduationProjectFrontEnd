...
class CardNotesAdmin extends StatefulWidget {
...
class CartCardState extends State<CardNotesAdmin> with Crud {
  onDelete() async {
    var response = await http.get(Uri.parse(linkIp + "/admin/deleteDoctorById?id="+widget.id));

    Navigator.of(context).pushNamedAndRemoveUntil("AdminDoctors", (route) => false);
  }
  onEdit() async {
    sharedPref.setString("isAdmin","1");
